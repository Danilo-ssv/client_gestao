...
import { DespesasServices } from '../services/despesas_services';
...
const DespesasState = create<State>()((set, get) => ({
  data: [],
  selectedItems: [],
  loading: 'none',
  search: '',
  ...
  read: async function () {
    ...
    const res = await new DespesasServices().read(state.search, state.page, state.limit);
  ...
  insert: async function (modelo: InsertDespesasModel) {
    set(() => ({ loading: 'savingInsert' }));

    const res = await new DespesasServices().insert(modelo);
  ...
  readById: async function (id: string) {
    ...
    const res = await new DespesasServices().readById(id);
