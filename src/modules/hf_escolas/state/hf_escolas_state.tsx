...
import { HfEscolasServices } from '../services/hf_escolas_services';
...
interface State {
  ...
  limit: number,
  read: () => void,
  insert: (modelo: InsertHfEscolasModel) => void,
  delete: (listaIds: number[]) => Promise<boolean>,
  readById: (id: string) => Promise<InsertHfEscolasModel | null>,
...
const HfEscolasState = create<State>()((set, get) => ({
  data: [],
  selectedItems: [],
  loading: 'none',
  search: '',
  ...
  read: async function () {
    ...
    const res = await new HfEscolasServices().read(state.search, state.page, state.limit);
  ...
  insert: async function (modelo: InsertHfEscolasModel) {
    set(() => ({ loading: 'savingInsert' }));

    const res = await new HfEscolasServices().insert(modelo);
  ...
  delete: async function (listaIds: number[]) {
    const res = await new HfEscolasServices().delete(listaIds);
  ...
  readById: async function (id: string) {
    ...
    const res = await new HfEscolasServices().readById(id);
