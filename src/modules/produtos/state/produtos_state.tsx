...
import { ProdutosServices } from "../services/produtos_services";
...
interface State {
  ...
  limit: number,
  read: () => void,
  insert: (modelo: InsertProdutosModel) => void,
  delete: (listaIds: string[]) => Promise<boolean>,
  readById: (id: string) => Promise<InsertProdutosModel | null>,
  insertBaixaEstoque: (idProdutos: string, estoque: number, dataBaixa: string) => Promise<boolean>,
...
const ProdutosState = create<State>()((set, get) => ({
  data: [],
  selectedItems: [],
  loading: 'none',
  search: '',
  ...
  read: async function () {
    ...
    const res = await new ProdutosServices().read(state.search, state.page, state.limit);
  ...
  insert: async function (modelo: InsertProdutosModel) {
    set(() => ({ loading: 'savingInsert' }));

    const res = await new ProdutosServices().insert(modelo);
  ...
  delete: async function (listaIds: string[]) {
    const res = await new ProdutosServices().delete(listaIds);
  ...
  readById: async function (id: string) {
    ...
    const res = await new ProdutosServices().readById(id);
  ...
  insertBaixaEstoque: async function (idProdutos: string, estoque: number, dataBaixa: string) {
    const res = await new ProdutosServices().insertBaixaEstoque(idProdutos, estoque, dataBaixa);
