...
export function InsertContasReceberPage() {
  ...
  const [controller, setController] = useState<InsertContasReceberModel>({
    id: '',
    idClientes: '0',
    nomeClientes: 'Selecione um Cliente',
    dataEmissao: '',
    dataVencimento: '',
    descricao: '',
    valor: '',
  ...
  function insert(event: React.FormEvent<HTMLFormElement>) {
    ...
    state.insert({
      ...
      idClientes: controller.idClientes,
      nomeClientes: controller.nomeClientes,
      ...
      <form onSubmit={insert} action="">
        <div className="grid grid-cols-2 gap-x-2">
          <div>
            ...
            <CustomSearchSelect
              id={controller.idClientes}
              title={controller.nomeClientes}
              idForLabel="idClientes"
              placeholder={'Pesquise um Cliente...'}
              selectItem={{ id: '0', title: 'Selecione um Cliente', inLineSubTitle: null, subTitle: null }}
              getItems={async (value) => {
                const res = await globalProvider.readClientes(value) ?? [];
