...
import { CategoriasProdutosState } from "../state/categorias_produtos_state";
...
export function CategoriasProdutosPage() {
  const localStorageProvider = new LocalStorageProvider();
  const state = CategoriasProdutosState();
    ...
    <>
      <div className="px-8 mt-1">
        ...
        <div className="flex justify-between">
          {
            !(localStorageProvider.getUser()?.permissoes.categoriasProdutosInsert ?? false)
              ? <div></div>
              : <NavLink to={ClientRoutes.dashboardCategoriasProdutosInserir('', false)}>
      ...
      <div className='mx-4 px-4 pb-4 bg-white relative'>
        <CustomTable
          data={state.data}
          columns={(item) => {
            const element = item as (CategoriasProdutosModel | null);
              ...
              },
              {
                ...
                value: <Popover>
                  ...
                  <PopoverContent className="p-0 w-40 bg-white border-1 border-gray-300 shadow-xl/20 rounded-[8px] relative -top-1 right-5">
                    {
                      !(localStorageProvider.getUser()?.permissoes.categoriasProdutosUpdate ?? false)
                        ? null
                        : <NavLink to={ClientRoutes.dashboardCategoriasProdutosInserir(element?.id ?? '', false)}>
                    ...
                    }
                    {
                      !(localStorageProvider.getUser()?.permissoes.categoriasProdutosInsert ?? false)
                        ? null
                        : <NavLink to={ClientRoutes.dashboardCategoriasProdutosInserir(element?.id ?? '', true)}>
                          <span className="m-1 py-1 px-2 rounded-[8px] hover:bg-gray-200 block">Clonar</span>
                        </NavLink>
                    }
                    {
                      !(localStorageProvider.getUser()?.permissoes.categoriasProdutosDelete ?? false)
                        ? null
                        : <div>
                          <hr />
                          <DeleteModal
