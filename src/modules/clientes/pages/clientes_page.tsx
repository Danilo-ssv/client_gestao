    ...
    <>
      <div className="px-8 mt-1">
        ...
        <div className="flex justify-between">
          {
            !(localStorageProvider.getUser()?.permissoes.clientesInsert ?? false)
              ? <div></div>
              : <NavLink to={ClientRoutes.dashboardClientesInserir('', false)}>
      ...
      <div className='mx-4 px-4 pb-4 bg-white relative'>
        <CustomTable
          data={state.data}
          columns={(item) => {
            const element = item as (ClientesModel | null);
              ...
              },
              {
                ...
                value: <Popover>
                  ...
                  <PopoverContent className="p-0 w-40 bg-white border-1 border-gray-300 shadow-xl/20 rounded-[8px] relative -top-1 right-5">
                    {
                      !(localStorageProvider.getUser()?.permissoes.clientesUpdate ?? false)
                        ? null
                        : <NavLink to={ClientRoutes.dashboardClientesInserir(element?.id ?? '', false)}>
                    ...
                    }
                    {
                      !(localStorageProvider.getUser()?.permissoes.clientesInsert ?? false)
                        ? null
                        : <NavLink to={ClientRoutes.dashboardClientesInserir(element?.id ?? '', true)}>
                          <span className="m-1 py-1 px-2 rounded-[8px] hover:bg-gray-200 block">Clonar</span>
                        </NavLink>
                    }
                    {
                      !(localStorageProvider.getUser()?.permissoes.clientesDelete ?? false)
                        ? null
                        : <div>
                          <hr />
                          <DeleteModal
