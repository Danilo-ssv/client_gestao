...
import { ClientesServices } from '../services/clientes_services';
...
interface State {
  ...
  limit: number,
  read: () => void,
  insert: (modelo: InsertClientesModel) => void,
  delete: (listaIds: string[]) => Promise<boolean>,
  readById: (id: string) => Promise<InsertClientesModel | null>,
...
const ClientesState = create<State>()((set, get) => ({
  data: [],
  selectedItems: [],
  loading: 'none',
  search: '',
  ...
  read: async function () {
    ...
    const res = await new ClientesServices().read(state.search, state.page, state.limit);
  ...
  insert: async function (modelo: InsertClientesModel) {
    set(() => ({ loading: 'savingInsert' }));

    const res = await new ClientesServices().insert(modelo);
  ...
  delete: async function (listaIds: string[]) {
    const res = await new ClientesServices().delete(listaIds);
  ...
  readById: async function (id: string) {
    ...
    const res = await new ClientesServices().readById(id);
